...
from .randaugment_video import TemporalConsistentRandomAugment
...
def create_random_augment(
    ...
    if auto_augment:
        ...
        if translate_const:
            aa_params["translate_const"] = translate_const
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, clip):
        ...
        if random.random() < 0.5:
            if isinstance(clip, torch.Tensor): # (T, H, W, C)
                if clip.shape[-1] == 3:
                    clip = clip.flip(-2)
                else:
                    clip = clip.flip(-1)
...
class RandomShortSideScale(object):
    """Random Resizes a list of (H x W x C) numpy.ndarray to the final size
    The larger the original image is, the more times it takes to
    interpolate
    Args:
    interpolation (str): Can be one of 'nearest', 'bilinear'
    ...
    def __init__(self, min_size, max_size, interpolation='nearest'):
        self._min_size = min_size
        self._max_size = max_size
        self.interpolation = interpolation

    def __call__(self, clip):
        size = random.randint(self._min_size, self._max_size + 1)
        h, w, c = clip[0].shape
        if w < h:
        ...
        resized = FF.resize_clip(
            clip, (new_w, new_h), interpolation=self.interpolation)
        return resized
...
class RandomResizedCrop(object):
    ...
    def __call__(self, clip):
        ...
        return FF.resized_crop(clip, i, j, h, w, self.size, self.interpolation_mode)
...
def random_multi_scale_crop(
    ...
    **kargs
    ...
    def _sample_crop_size(im_size, max_distort=1, fix_crop=True, more_fix_crop=True):
        image_w, image_h = im_size[0], im_size[1]

        # find a crop size
        base_size = min(image_w, image_h)
        crop_sizes = [int(base_size * x) for x in scales]
        crop_h = [target_width if abs(x - target_width) < 3 else x for x in crop_sizes]
        crop_w = [target_height if abs(x - target_height) < 3 else x for x in crop_sizes]
        ...
        if not fix_crop:
            ...
            offsets = fill_fix_offset(more_fix_crop, image_w, image_h, crop_pair[0], crop_pair[1])
            w_offset, h_offset = random.choice(offsets)

        return crop_pair[0], crop_pair[1], w_offset, h_offset

    h, w, i, j = _sample_crop_size((height, width))
    cropped = images[:, :, i : i + h, j : j + w]
    return torch.nn.functional.interpolate(
        cropped,
        size=(target_height, target_width),
...
TemporalConsistentRandomAugment = TemporalConsistentRandomAugment