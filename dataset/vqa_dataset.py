...
class vqa_dataset(Dataset):
    ...
    def get_item_length(self, index):
        ...
        length_count += len(question.split(' '))

        if self.add_ocr and "ocr" in ann:
            ocrs = ann['ocr']
            length_count += len(ocrs)
         
        if self.add_object and "object_label" in ann:
            objects = ann["object_label"]
            length_count += len(objects.split("&&"))

        if self.split=='train':                       
        
            if ann['dataset']=='vqa':
                max_ans_len = max([len(_.split(' '))for _ in ann['answer']])
            elif ann['dataset']=='vg':
                max_ans_len = len(ann['answer'].split(' '))
            elif ann['dataset']=='gqa':
                max_ans_len = len(ann['answer'].split(' '))
            length_count += max_ans_len
        return length_count
